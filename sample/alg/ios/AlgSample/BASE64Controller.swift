...
class BASE64Controller: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        tvEncDest = UILabel(frame: CGRect(x: 8, y: 64 + 88, width: w - 16, height: 32))
        btnEncGo = UIButton(type: UIButtonType.system)
        btnEncGo?.frame = CGRect(x: 8, y: 64 + 128, width: w - 16, height: 32)
        btnEncGo?.backgroundColor = self.view.tintColor
        btnEncGo?.setTitleColor(UIColor.white(), for: [])
        btnEncGo?.setTitle(STR_BTN_GO, for: [])
        
        tvDecTitle = UILabel(frame: CGRect(x: 8, y: 64 + 168, width: w - 16, height: 32))
        tvDecTitle?.text = STR_DEC
        etDecSrc = UITextField(frame: CGRect(x: 8, y: 64 + 208, width: w - 16, height: 32))
        etDecSrc?.borderStyle = UITextBorderStyle.roundedRect
        etDecSrc?.placeholder = STR_DEC_HINT
        tvDecDest = UILabel(frame: CGRect(x: 8, y: 64 + 248, width: w - 16, height: 32))
        btnDecGo = UIButton(type: UIButtonType.system)
        btnDecGo?.frame = CGRect(x: 8, y: 64 + 288, width: w - 16, height: 32)
    ...
    func btnClicked(sender: AnyObject?) {
        ...
        if (btn == btnEncGo) {
            ...
            let enc = base64EncryptString(ori)
            ...
            let dec = base64DecryptString(ori)
