...
function _rsassaGenerateKeys(keySize: Integer; pubPass: PChar; pubPath: PChar; privPass: PChar; privPath: PChar): Integer; stdcall;
...
function rsassaGenerateKeys(keySize: Integer; pubPass: PChar; pubPath: PChar; privPass: PChar; privPath: PChar): Integer; stdcall;
...
function Java_com_hujiang_devart_security_AlgorithmUtils_rsassaGenerateKeys(env: PJNIEnv; obj: jobject; keySize: jint; pubPass: jstring; pubPath: jstring; privPass: jstring; privPath: jstring): jint; stdcall;
...
function _rsassaGenerateKeys(keySize: Integer; pubPass: PChar; pubPath: PChar;
  privPass: PChar; privPath: PChar): Integer; stdcall;
var
  rsassa: TLbRSASSA;
begin
  Result := -1;
  ...
  try
    ...
    rsassa.PublicKey.StoreToFile(string(pubPath));
    rsassa.PrivateKey.Passphrase:= string(privPass);
    rsassa.PrivateKey.StoreToFile(string(privPass));
    Result := 0;
  finally
    rsassa.Free;
  end;
...
function rsassaGenerateKeys(keySize: Integer; pubPass: PChar; pubPath: PChar;
  privPass: PChar; privPath: PChar): Integer; stdcall;
begin
  Result := _rsassaGenerateKeys(keySize, pubPass, pubPath, privPass, privPath);
end;
...
function Java_com_hujiang_devart_security_AlgorithmUtils_rsassaGenerateKeys(
  ...
  pubPath: jstring; privPass: jstring; privPath: jstring): jint; stdcall;
begin
  Result := _rsassaGenerateKeys(keySize, PChar(jstringToString(env, pubPass)), PChar(jstringToString(env, pubPath)), PChar(jstringToString(env, privPass)), PChar(jstringToString(env, privPath)));
end;
